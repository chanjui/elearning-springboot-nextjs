...
public interface CourseEnrollmentRepository extends JpaRepository<CourseEnrollment, Long> {
  ...
  @Query("SELECT ce FROM CourseEnrollment ce JOIN FETCH ce.payment WHERE ce.user.id = :userId AND ce.completionStatus = false")
  List<CourseEnrollment> findByUserIdAndCompletionStatusFalse(@Param("userId") Long userId);

  @Query("SELECT ce FROM CourseEnrollment ce JOIN FETCH ce.payment WHERE ce.user.id = :userId AND ce.completionStatus = true")
