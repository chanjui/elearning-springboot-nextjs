...
public class CourseCreateController {
  ...
  public ResponseEntity<String> saveCurriculum(
    ...
    courseService.updateCurriculum(request);
  ...
  public ResponseEntity<Page<CourseResponseDTO>> getInstructorCourses(
      ...
      @RequestParam(required = false) String status, // ✅ status 추가
      @RequestParam(required = false) String keyword, // ✅ keyword도 추가 (이미 프론트에 존재하니까)
    ...
    Page<CourseResponseDTO> result = courseService.getCoursesByInstructor(instructorId, pageable, status, keyword); // ✅
                                                                                                                    // 전달
  ...
  public ResponseEntity<?> closeCourse(@PathVariable Long id) {
    courseService.closeCourse(id);
    return ResponseEntity.ok("강의가 종료(Closed) 처리되었습니다.");
  }
}