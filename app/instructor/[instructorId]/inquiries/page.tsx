...
export default function InstructorInquiriesPage() {
  ...
  const fetchInquiries = async () => {
    ...
    if (filterCourse !== "전체") params.append("courseId", filterCourse)

    const res = await fetch(`/api/instructor/inquiries/${instructorId}?${params.toString()}`)
              ...
              {inquiries.map((inquiry) => (
                <div key={inquiry.id} className="border border-gray-800 rounded-lg overflow-hidden">
                  <div className="bg-gray-800 p-4">
                    ...
                    <div className="text-sm text-gray-400 mt-1">{inquiry.author} • {inquiry.regDate}</div>
                  </div>
                  ...
                  <div className="p-4 bg-gray-900">
                    ...
                    {inquiry.replies.map((reply) => (
                      <div key={reply.id} className="bg-gray-800 p-3 rounded-lg mb-2">
                        <div className="flex justify-between">
                          ...
                          <div className="text-gray-400 text-sm">{reply.regDate}</div>
                        </div>
                        ...
                        {editingReplyId === reply.id ? (
                          <>
                            ...
                            <div className="flex justify-end gap-2 mt-2">
                              ...
                              <Button onClick={() => handleUpdateReply(reply.id)}>수정완료</Button>
                            </div>
                          </>
                        ) : (
                        ...
                        <div className="flex justify-end gap-2 mt-2">
                          <Button size="sm" variant="outline" onClick={() => { setEditingReplyId(reply.id); setEditReplyContent(reply.content) }}>수정</Button>
                          <Button size="sm" variant="outline" onClick={() => handleDeleteReply(reply.id)}>삭제</Button>
                        </div>
                      </div>
                    ))}
                    ...
                    {inquiry.replyCount === 0 && replyingTo === inquiry.id && (
                      <>
                        ...
                        <div className="flex justify-end gap-2 mt-2">
                          ...
                          <Button onClick={() => handleReplySubmit(inquiry.id)}>답변 제출</Button>
                        </div>
                      </>
                    )}
                    ...
                    {inquiry.replyCount === 0 && replyingTo !== inquiry.id && (
                      <div className="flex justify-end">
                        <Button onClick={() => setReplyingTo(inquiry.id)}>답변하기</Button>
                      </div>
                    )}
                  </div>
                </div>
