...
//   // 전화 번호 모달
//   useEffect(() => {
//     if (userMainData && !userMainData.existPhone) {
...
//     await axios.post("/api/user/v1/updatePhone", { phone: phoneInput })
//     setShowPhoneModal(false)
//     location.reload()  // 새로고침으로 다시 메인 데이터 로딩
//   }
...
export default function UserHomePage() {
  ...
  useEffect(() => {
    axios.get("/api/course/main", { withCredentials: true })
      .then(res => {
        const data = res.data.data
        setUserMainData(data)
        setExistCourse(data.existCourse)
